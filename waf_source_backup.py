    ...
    def __init__(self, callbacks, helpers, req_delay, message, vuln_type, bounds):
        ...
        try:
            ...
            self._orig_is_json = any(
                "content-type:" in _h.lower() and "application/json" in _h.lower()
                for _h in _init_hdrs
            )
    ...
    def _blind_extract_html(self, ext_data):
        ...
        if 'db_name' in ext_data:
            rows += ("<tr><td><b>Database</b></td>"
                     "<td><code>%s</code></td></tr>" % ext_data['db_name'][0])
        ...
        for key in ('table_1', 'table_2'):
            if key in ext_data:
                ...
                rows += ("<tr><td><b>table[%d]</b></td>"
                         "<td><code>%s</code></td></tr>" % (idx, ext_data[key][0]))
            ...
            if err_result:
                ...
                if "SQL Injection" not in self._issued_high:
                    ...
                    self._add_issue(
                        ...
                        % (chain_name, edb, epayload[:300], ebody[:400]),
                        severity="High",
                        confidence="Certain",
                    )
                ...
                if time_result:
                    ...
                    if "SQL Injection" not in self._issued_high:
                        ...
                        self._add_issue(
                            ...
                            % (chain_name, tdb, tpayload[:300], telapsed),
                            severity="High",
                            confidence="Certain",
                        )
